...
int main(int argc, char *argv[]){
	...
	if(argc==1){
		...
		if(strcmp("delete",argv[1])==0){
			SC_HANDLE hSc = OpenService(hScm, "gigaso", DELETE);
		...
		if(strcmp("start",argv[1])==0){
			SC_HANDLE hSc = OpenService(hScm,  "gigaso", SERVICE_ALL_ACCESS);
		...
		if(strcmp("create",argv[1])==0){
			SC_HANDLE hSc = CreateService (hScm, "gigaso", "Gigaso File Searcher",
